...
public class AuditlogServices : IAuditlogServices
{
  ...
  public async Task RegisterLoginActionAsync(bool succes, int? userId, string recordDisplay)
  {
    await RegisterActionAsync(
        ...
        // CORREGIDO: Usamos el tipo de acción correcto según el éxito
        actionType: succes ? ActionType.LOGIN_SUCCESS : ActionType.LOGIN_FAILURE,
        recordDisplay: recordDisplay,
        recordId: userId ?? 0,
        status: succes ? AuditStatus.SUCCESS : AuditStatus.FAILURE,
        changeDetail: succes ? "Login exitoso" : "Fallo de inicio de sesión - Credenciales inválidas"
    );
  }
